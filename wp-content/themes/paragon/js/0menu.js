...
let isBigImage55 = document.querySelector(".big-images").parentNode
if(isBigImage55){
    isBigImage55.style.cssText = 'min-width:100vw;'
}

let isAbout = document.querySelector("#about").children
if(isAbout){
    isAbout[0].classList.add('container')
}
let isServices = document.querySelector("#our_services").children
if(isServices){
    isServices[0].classList.add('container')
}
let isProperties = document.querySelector("#properties").children
if(isProperties){
    isProperties[0].classList.add('container')
}
let isNews = document.querySelector("#market_news").children
if(isNews){
    isNews[0].classList.add('container')
}
let newSizeOfScreen = document.documentElement.clientWidth
if(newSizeOfScreen>768) {
    let isNewsBox = document.querySelector(".home-news-box").children
    if (isNewsBox) {
        isNewsBox[0].classList.add('container')
    }
}
window.onresize = () => {
    newsBox()
...
function newsBox(){
    ...
    if(newSizeOfScreen2<=768){
        let isNewsBox2 = document.querySelector(".home-news-box").children
        if(isNewsBox2){
            isNewsBox2[0].classList.remove('container')
        }
    }
}